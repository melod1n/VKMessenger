...
public class VKUser implements Serializable {
    ...
    public static VKUser parseFromNews(JSONObject jprofile) throws JSONException {
        VKUser m = new VKUser();
        m.uid = jprofile.getLong("id");
        m.first_name = VKApi.unescape(jprofile.optString("first_name"));
        m.last_name = VKApi.unescape(jprofile.optString("last_name"));
    ...
    public static VKUser parseFromGetByPhones(JSONObject o) throws JSONException {
        VKUser u = new VKUser();
        u.uid = o.getLong("id");
        u.first_name = VKApi.unescape(o.optString("first_name"));
        u.last_name = VKApi.unescape(o.optString("last_name"));
    ...
    public static ArrayList<VKUser> parseUsers(JSONArray array, boolean from_notifications) throws JSONException {
        ...
        for(int i=0; i<category_count; ++i){
            ...
            if(from_notifications)
                u = VKUser.parseFromNotifications(o);
            else
                u = VKUser.parse(o);
            users.add(u);
        }
    ...
    public static ArrayList<VKUser> parseUsersForGetByPhones(JSONArray array) throws JSONException {
        ArrayList<VKUser> users=new ArrayList<VKUser>();
        //it may be null if no users returned
        //no users may be returned if we request users that are already removed
        if(array==null)
        ...
        for(int i=0; i<category_count; ++i){
            if(array.get(i)==null || ((array.get(i) instanceof JSONObject)==false))
                continue;
            JSONObject o = (JSONObject)array.get(i);
            VKUser u = VKUser.parseFromGetByPhones(o);
            users.add(u);
        }
        return users;
    }
    
    public static VKUser parseFromFave(JSONObject jprofile) throws JSONException {
        VKUser m = new VKUser();
    ...
    public static VKUser parseFromNotifications(JSONObject jprofile) throws JSONException {
        VKUser m = new VKUser();
        m.uid = jprofile.getLong("id");
        m.first_name = VKApi.unescape(jprofile.optString("first_name"));
        m.last_name = VKApi.unescape(jprofile.optString("last_name"));
